  ...
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5">
        {/* Category */}
        <p className="text-[10px] text-[#D4A59A] font-semibold uppercase tracking-widest mb-1">{product.category}</p>

        {/* Name */}
        <h3 className="text-sm font-semibold text-[#6B4423] leading-tight mb-2 line-clamp-2">{product.name}</h3>
          ...
          <div className="flex items-center gap-0.5">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                size={10}
                className={i < Math.floor(product.rating) ? 'text-[#D4A59A] fill-[#D4A59A]' : 'text-[#E8D4C4]'}
              />
            ))}
          </div>
          <span className="text-[10px] text-[#B89B8A]">({product.reviews.toLocaleString()})</span>
        </div>

        {/* Price */}
        <div className="flex items-center gap-2 mb-4">
          <span className="text-lg font-bold text-[#6B4423]">${product.price} MXN</span>
          {product.originalPrice && (
            <>
              <span className="text-xs text-[#B89B8A] line-through">${product.originalPrice}</span>
              <span className="text-[10px] font-bold text-[#D4A59A] bg-[#FDF0ED] px-2 py-0.5 rounded-full">-{discount}%</span>
            </>
          )}
        </div>
